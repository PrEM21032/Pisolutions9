const DOMAIN_RULES = Object.freeze([
  { name: 'weather', pattern: /weather|forecast|temperature|rain|snow|wind|humidity|climate today/i, tasks: ['identify_location', 'retrieve_current_weather', 'summarize_weather'] },
...
const CUSTOMER_LABELS = Object.freeze({
  weather: ['Weather', 'Retrieve current weather for the selected location and summarize it clearly.']
});
...
export function planNoGpt(objective = '') {
  ...
  const customerIntent = domains.length === 1 && CUSTOMER_LABELS[domains[0]] ? domains[0] : null;
  return Object.freeze({ mode: 'no-gpt', objective: text, domains, tasks, planner: 'deterministic-rule-engine', customerIntent });
}

export function executeNoGptPlan(plan) {
  if (!plan || plan.mode !== 'no-gpt') throw new Error('no_gpt_plan_required');
  const customer = plan.customerIntent && CUSTOMER_LABELS[plan.customerIntent];
  return {
    ...
    customerResponse: customer ? { title: customer[0], message: customer[1], dataRequired: 'current external weather data' } : null,
