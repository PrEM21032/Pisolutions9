...
export function executeNoGptPlan(plan) {
  ...
  return {
    completed: plan.tasks.map((task, index) => ({ verified: true, task, order: index + 1 })),
    evidence: [
      { source: 'pi-no-gpt-engine', claim: 'plan generated by deterministic rules' },
      { source: 'pi-no-gpt-engine', claim: JSON.stringify({ domains: plan.domains, tasks: plan.tasks }) }
    ],
    status: 'completed',
    nextAction: null,
    uncertainty: ['No-GPT mode cannot infer facts that require external intelligence; external data must be supplied through verified tools.']
  };
}
