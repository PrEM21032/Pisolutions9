...
export default{async fetch(request,env){const url=new URL(request.url);const origin=request.headers.get('Origin')||'';if(url.pathname==='/api/session')return handleSessionRequest(request,env,ALLOWED_ORIGIN);if(url.pathname!=='/api/chat')return new Response('Not found',{status:404});if(origin&&origin!==ALLOWED_ORIGIN)return json({ok:false,error:'origin_not_allowed'},403,request);if(request.method==='OPTIONS')return preflight(request);if(request.method!=='POST')return json({ok:false,error:'method_not_allowed'},405,request);let payload;try{payload=await request.json();}catch{return json({ok:false,error:'invalid_json'},400,request);}const message=String(payload?.message||'').trim();if(!message)return json({ok:false,error:'message_required'},400,request);if(message.length>MAX_INPUT)return json({ok:false,error:'message_too_large'},413,request);let history=[];let attachment=null;let attachmentInfo=null;try{history=validateHistory(payload.history);attachment=validateAttachment(payload.attachment);if(!attachment){const mission=inventoryMission(message);if(mission)return json(mission,200,request);}if(attachment)attachmentInfo=await attachmentContext(env,attachment);}catch(error){const code=String(error?.message||error);const status=code==='attachment_conversion_unavailable'||code==='attachment_conversion_failed'?503:400;return json({ok:false,error:code},status,request);}const effectiveMessage=withAttachment(message,attachmentInfo);const conversationContext=recentConversationContext(history,message);if(requiresLiveEvidence(conversationContext)){if(env.OPENAI_API_KEY){const models=[...new Set([env.PI_WEB_MODEL,env.PI_CHAT_MODEL,...OPENAI_MODEL_FALLBACKS].filter(Boolean))];let lastFailure=null;for(const model of models){try{const response=await callProvider({apiKey:env.OPENAI_API_KEY,model,baseUrl:'https://api.openai.com/v1',message:effectiveMessage,history,useWebSearch:true,instructions:`${PI_INSTRUCTIONS} This request requires current information. Use web search, distinguish sourced facts from uncertainty, and do not claim freshness beyond the retrieved evidence.`});if(!response.ok){const failure=await providerError(response);lastFailure={response,failure,model};if(response.status===429||response.status>=500||response.status===404)continue;return json({ok:false,status:'live_evidence_required',error:failure.error,answer:'PI could not verify current information from the live research provider, so it will not guess.',truth:'unknown'},failure.status,request,rateLimitHeaders(response));}const body=await response.json();const answer=extractAnswer(body);if(!answer){lastFailure={failure:{error:'empty_live_research_response',status:503},model};continue;}const sources=extractSources(body);if(!sources.length&&!/\?\s*$/.test(answer)){lastFailure={failure:{error:'live_research_unverified',status:503},model};continue;}return json({ok:true,status:'answered',answer,source:`pi-chat-web:${model}`,truth:sources.length?'web-grounded-model-response':'model-response',sources},200,request);}catch(error){lastFailure={failure:{error:'live_research_provider_unavailable',status:503},model,error};}}const failure=lastFailure?.failure||{error:'live_research_provider_unavailable',status:503};const headers=lastFailure?.response&&failure.error==='chat_provider_rate_limited'?rateLimitHeaders(lastFailure.response):{};return json({ok:false,status:'live_evidence_required',error:failure.error,answer:'PI could not verify current information from the available live research providers, so it will not guess.',truth:'unknown'},failure.status||503,request,headers);}return json({ok:false,status:'live_evidence_required',error:'live_data_connector_not_configured',answer:'I need a live data source to answer that accurately. I will not guess or present model memory as current data.',truth:'unknown'},503,request);}const hardReasoning=requiresHardReasoning(effectiveMessage);
if(hardReasoning){
